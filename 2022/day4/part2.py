...
def are_intersecting(a: list, b: list) -> bool:
    a_set: set = set(range(a[0], a[1] + 1))
    b_set: set = set(range(b[0], b[1] + 1))
    return a_set.intersection(b_set)


def solve_problem(input_file) -> int:

    intersecting_pairings = 0
    ...
    for pairing in read_input(input_file):
        ...
        if are_intersecting(a, b):
            intersecting_pairings += 1
            print(f"{intersecting_pairings:4} {a} {b}")

    return intersecting_pairings


print(f"== day 4 part 2 example", "=" * 40)
example_answer = solve_problem("input.example.txt")
assert example_answer == 4, f"calculated answer: {example_answer}"

print(f"== day 4 part 2 problem", "=" * 40)
